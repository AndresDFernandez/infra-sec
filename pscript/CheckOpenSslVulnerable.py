    ...
    def startCheckVulnerability(self,ip,hostname):
        try:
            ...
            if result == 0:
                ...
                print '\n###### Started scanning for checking OPENSSL Heartbleed vulnerability \'%s\' ######\n' % (ip)
            
                #check heartbeat vulnerability in port 443
                self.checkVulnerability(ip,'443')

                print '\n###### Started scanning for checking OPENSSL Heartbleed vulnerability \'%s\' ######\n' % (hostname)
            
                #check heartbeat vulnerability in port 443
                self.checkVulnerability(hostname,'443')
